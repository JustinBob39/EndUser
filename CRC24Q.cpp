...
const unsigned int CRC24Q::crc24q[256] = {
...
unsigned int CRC24Q::crc24q_hash(const unsigned char *data, const int len) {
    unsigned int crc = 0;
    for (int i = 0; i < len; i++) {
        crc = (crc << 8) ^ CRC24Q::crc24q[data[i] ^ static_cast<unsigned char>(crc >> 16)];
    }
    crc = crc & 0x00ffffff;
    return crc;
}

bool CRC24Q::crc24q_check(const unsigned char *data, const int len) {
    const unsigned int crc = CRC24Q::crc24q_hash(data, len - 3);
